...
pop_dist <- sim_abundance(ages = 1:10, years = 1:15,
  ...
  sim_distribution(grid = g,
                            ...
                            depth_par = sim_parabola(mu = 100,
                                                     sigma = 50,
                                                     sigma_right = 20))
plot_distribution(pop_dist, ages = 1:3, years = 1:3, type = "heatmap")
```
...
dat <- as_tibble(sim_lo$setdet) %>%
  select(x, y, set, year, N = n, tow_area) %>%
  left_join(., xy, by = c("x", "y")) %>%
  mutate(offset = log(tow_area))

## Combine indices
true_abund <- tibble(year = unique(dat$year), N = as.numeric(colSums(sim_lo$I))) %>% mutate(type = "True")
